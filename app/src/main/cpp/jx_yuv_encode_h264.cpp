...
//        av_dict_set(&param, "profile", "baseline", 0);
    ...
    av_dump_format(pFormatCtx, 0, out_file, 1); //Show some Information
...
void *JXYUVEncodeH264::startEncode(void *obj) {
    ...
    while (!h264_encoder->is_end||!h264_encoder->frame_queue.empty()) {
        if(h264_encoder->is_release){
            av_write_trailer(h264_encoder->pFormatCtx); //Write file trailer
            //Clean
 ...
 */
void
JXYUVEncodeH264::custom_filter(const JXYUVEncodeH264 *h264_encoder, const uint8_t *picture_buf, int in_y_size, int format) {
    int y_height_start_index=h264_encoder->arguments->in_height-h264_encoder->arguments->out_height; // y值在H方向开始行
    int uv_height_start_index=y_height_start_index/2; // uv值在H方向开始行
...
int JXYUVEncodeH264::encodeEnd() {
    int ret_1 = flush_encoder(pFormatCtx, 0); //Flush Encoder
    if (ret_1 < 0) {
        LOGE(JNI_DEBUG, "Flushing encoder failed\n");
        return -1;
    }
    av_write_trailer(pFormatCtx); //Write file trailer
