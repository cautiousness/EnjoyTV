...
public class MediaObject implements Serializable {
    private final static int MEDIA_PART_TYPE_RECORD = 0; // 拍摄
    private final static int MEDIA_PART_TYPE_IMPORT_VIDEO = 1; // 导入视频
    private final static int MEDIA_PART_TYPE_IMPORT_IMAGE = 2; // 导入图片
    private final static int MEDIA_PART_TYPE_RECORD_MP4 = 3; // 使用系统拍摄mp4
    private final static int DEFAULT_MAX_DURATION = 10 * 1000; // 默认最大时长
    private final static int DEFAULT_VIDEO_BITRATE = 800; // 默认码率
    ...
    private volatile transient MediaPart mCurrentPart; // 当前分块
    private LinkedList<MediaPart> mMediaList = new LinkedList<>(); // 获取所有分块
    public MediaThemeObject mThemeObject; // 主题
    ...
    public String getObjectFilePath() {
        if (StringUtils.isEmpty(mOutputObjectPath)) {
            ...
            mOutputObjectPath = mOutputDirectory + File.separator + f.getName() + ".obj";
        }
    ...
    public static class MediaPart implements Serializable {
        public int index; // 索引
        public String mediaPath; // 视频路径
        public String audioPath; // 音频路径
        public String tempMediaPath; // 临时视频路径
        public String tempAudioPath; // 临时音频路径
        public String thumbPath; // 截图路径
        public String tempPath; // 存放导入的视频和图片
        public int type = MEDIA_PART_TYPE_RECORD; // 类型
        public int cutStartTime; // 剪切视频（开始时间）
        public int cutEndTime; // 剪切视频（结束时间）
        public int duration; // 分段长度
        public int position; // 总时长中的具体位置
        public int speed = 10; // 0.2倍速-3倍速（取值2~30）
        public int cameraId; // 摄像头
        public int yuvWidth; // 视频尺寸
        public int yuvHeight; // 视频高度
