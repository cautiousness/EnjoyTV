...
public abstract class MediaRecorderBase implements Callback, PreviewCallback, IMediaRecorder {
    ...
    public static int SMALL_VIDEO_HEIGHT = 480; // 小视频高度
    public static int SMALL_VIDEO_WIDTH = 360; // 小视频宽度
    ...
    protected int mFrameRate = MAX_FRAME_RATE; // 帧率
    public int mCameraId = Camera.CameraInfo.CAMERA_FACING_BACK; // 摄像头类型（前置/后置），默认后置
    public static int mVideoBitrate; // 视频码率
    ...
    private void prepareCameraParaments() {
        ...
        mParameters.setPreviewFormat(ImageFormat.YV12); // 设置输出视频流尺寸，采样率

        String mode = getAutoFocusMode(); //设置自动连续对焦
        if (StringUtils.isNotEmpty(mode)) {
            mParameters.setFocusMode(mode);
        }
    ...
    private void setPreviewCallback() {
        ...
        if (size != null) {
            ...
            LogUtils.i("width:" + size.width + " height:" + size.height);
        } else {
    ...
    public void release() {
        ...
        if (mAudioRecorder != null) {
            mAudioRecorder.interrupt(); // 停止音频录制
            mAudioRecorder = null;
        }
        mSurfaceHolder = null;
        mPrepared = false;
    ...
    public interface OnErrorListener {
        ...
        void onVideoError(int what, int extra); // 视频录制错误

        void onAudioError(int what, String message); // 音频录制错误
    }
    ...
    public interface OnEncodeListener {
        ...
        void onEncodeStart(); // 开始转码

        void onEncodeProgress(int progress); // 转码进度

        void onEncodeComplete(); // 转码完成

        void onEncodeError(); // 转码失败
    }
    ...
    protected Boolean doCompress(boolean mergeFlag) {
        if (compressConfig != null) {
            ...
            String cmd_transcoding = String.format("ffmpeg -threads 16 -i %s -c:v libx264 %s %s %s -c:a libfdk_aac %s %s %s %s",
                    ...
                    vbr, getFrameRateCmd(), scaleWH, mMediaObject.getOutputTempTranscodingVideoPath()
            );
    ...
    private String getFrameRateCmd() {
        return mFrameRateCmd;
    }

    protected void setTranscodingFrameRate(int rate) {
        this.mFrameRateCmd = String.format(Locale.CHINESE, " -r %d", rate);
    }
