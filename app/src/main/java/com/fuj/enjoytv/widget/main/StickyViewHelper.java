 ...
 * 也可以更具自己需求复写某些方法
 */
public class StickyViewHelper implements View.OnTouchListener, BubbleView.DragStickViewListener {
    ...
    private BubbleView mStickyView;
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
          ...
          */
            ...
            mStickyView = new BubbleView(mContext, mDragView, mWm);
            initStickyViewData(); // 初始化数据
            mStickyView.setDragStickViewListener(this); // 注册拖拽过程的监听回调
            mWm.addView(mStickyView, mParams); // 开始添加的窗体让其显示
    ...
    private void initStickyViewData() {
        int[] points = new int[2];  // 计算小圆点在屏幕的坐标
        mShowView.getLocationInWindow(points);
        int x = points[0] + mShowView.getWidth() / 2;
        int y = points[1] + mShowView.getHeight() / 2;
        mStickyView.setStatusBarHeight(mStatusBarHeight); // 需要外部设置，当StickyView还没有执行完dispatchAttachedToWindow()时是计算不出其高度的
        ...
        mStickyView.setShowCanterPoint(x, y); // 初始化做作画的圆和控制点坐标
    }
    ...
    private void playAnim(PointF dragCanterPoint) {
        ...
        int intrinsicWidth = imageView.getDrawable().getIntrinsicWidth(); // 这里得到的是其真实的大小，因为此时还得不到其测量值
        ...
        long duration = getAnimDuration(mAnimDrawable); // 获取播放一次帧动画的总时长
        mWm.addView(imageView, mParams);
        mAnimDrawable.start();

        imageView.postDelayed(new Runnable() { // 由于帧动画不能定时停止，只能采用这种办法
            @Override
            public void run() {
                mAnimDrawable.stop();
